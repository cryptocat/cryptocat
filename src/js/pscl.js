...
ProScript.crypto = {
	...
	AESGCMEncrypt: function(k, iv, m, aad) {
		var aes = ProScript.nodeCrypto.createCipheriv(
	...
	AESGCMDecrypt: function(k, iv, m, aad) {
		var aes = ProScript.nodeCrypto.createDecipheriv(
			'aes-256-gcm', new Buffer(k), new Buffer(iv)
		);
		var res = '';
		aes.setAAD(new Buffer(aad, 'hex'));
	...
	SHA256: function(m) {
		var s = ProScript.nodeCrypto.createHash('sha256');
		return s.update(m, 'hex').digest('hex');
	},
	SHA512: function(m) {
		var s = ProScript.nodeCrypto.createHash('sha512');
		return s.update(m, 'hex').digest('hex');
	},
	HMACSHA256: function(k, m) {
		var hmac = ProScript.nodeCrypto.createHmac(
			'sha256', new Buffer(k)
		);
		hmac.update(m, 'utf8');
